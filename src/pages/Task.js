...
import '../style.scss';
import { fetchDailyTask, fetchTask } from '../Utils.js';
...
const Task = () => {
    const { MIN_DATE, MAX_DATE } = { MIN_DATE: "2023-01-01", MAX_DATE: fetchDailyTask().date };
    ...
    if ( !date || date < MIN_DATE || date > MAX_DATE ) {
        ...
        task = { title : "Task not found", verse : randomVerse };
    } else {
